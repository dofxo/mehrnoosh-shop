...
const discountAmount = Math.round(
  ((Number(item.price) - Number(item.discount_price)) / Number(item.price)) * 100
);
				return (        <div className="rounded-primary bg-white px-5 py-4" key={idx}>
          ...
          {/* Product actions */}
          <ProductActions languageData={languageData} discountAmount={discountAmount} />
        </div>
)
			})}
    </section>
